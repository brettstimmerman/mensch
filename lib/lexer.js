...
function lex(css) {
  ...
  function look(str) {
    var start = cursor + 1;
    var end = cursor + str.length + 1;

    return (str === css.slice(start, end));
  }
  ...
  // function error(msg) {
  //     return new Error(msg + ' Line ' + line + ', column ' + column);
  // }
    ...
    case '}':
      ...
      case 'selector':
        if (depth > 0) {
          ...
          // XXX: What if depth > 2?
          if (2 === depth) {
            popState(); // XXX: What is the state after this?
          }
        }
