...
function lex(css) {
  ...
  function getState(index) {
    //ANGUS not sure why index counts backwards
   ...
   */
  //ANGUS feels like this should be called nextStr and next should be nextChar
  function look(str) {
    var start = cursor + 1;

    //ANGUS "return find(str) == cursor + 1" instead?
   ...
   */
  //ANGUS feels like this should be called nextChar and look should be nextString
  function next(ch) {
    return ch === peek(1);
  }
  ...
  function pushState(newState) {
    state = newState;
    stack.push(state);
    //ANGUS "return stack.length" to be consistent with Array.prototype.push?
  }
