...
function stringifyAtGroup(node) {
  ...
  return '@' + prefix + node.type + label + _s + '{' + body + '}';
}
...
function stringifyRule(node) {
  ...
  if (node.selectors) {
    label = node.selectors.join(',');
  } else {
    label = '@' + node.type;
    label += node.name ? ' ' + node.name : '';
  }

  return label + _s + '{' + body + '}';
}
...
function stringifyBlock(node) {
  ...
  return stringifyChildren(children, fn);
}
...
function stringifyNode(node) {
  ...
  case 'import'  :
  case 'charset' : return stringifyAtRule(node);

  case 'font-face':
  case 'supports' :
  case 'viewport' : return stringifyAtGroup(node);
  }
